...
def renderizar_importacao_dxf() -> list[dict[str, Any]]:
    ...
    st.caption('Envie o DXF e use o editor na propria pagina para desenhar cada comodo diretamente sobre a planta.')
    ...
    if st.session_state.get(_session_key('file_hash')) != file_hash:
        ...
        st.session_state[open_key] = True
        st.session_state[_session_key('show_editor_inline')] = True
    ...
    action_col1, action_col2 = st.columns([1.2, 2.4])
    with action_col1:
        mostrar_editor = st.toggle(
            'Mostrar editor DXF na pagina',
            value=st.session_state.get(open_key, True),
            key=_session_key('show_editor_inline'),
        )
        st.session_state[open_key] = bool(mostrar_editor)
    with action_col2:
        st.info('No editor abaixo, ajuste a vista, desenhe o comodo, salve e informe o nome na hora.')
